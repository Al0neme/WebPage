...
# 删除链接方法，
def del_link(category_text, link_title):
    ...
    for i in category_dict[category_text]:
        if link_title in i:
            category_dict[category_text].remove(i)
...
def page_update(category_text, link_title):
    # 读取模板
    f_head = []
    ...
    category_dict = del_link(category_text, link_title)
