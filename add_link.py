...
# 添加链接方法，
def add_link(category_text,link_title,link_text):
    with open("category_dict.txt","r",encoding="utf-8") as fr:
        r = fr.readlines()[0]
    category_dict = ast.literal_eval(r) # 字符串转字典

    category_dict[category_text].append({link_title:link_text})
...
def page_update(category_text,link_title,link_text):
    ...
    category_dict = add_link(category_text,link_title,link_text)
